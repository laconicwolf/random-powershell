...
function Get-SingleCharXorBruteForce($ciphers) {
    ...
    $candidates = @()
    $ciphers = $ciphers | Convert-HexStringToByteArray
    foreach($key_candidate in 1..256){
        $plaintext_candidate = Get-SingleCharXor $ciphers $key_candidate
...
function Detect-EncryptedText($encrypted_strings) {
    <#Performs a singlechar XOR brute force attack to every 
    ciphertext of the input, gets a plaintext from each of 
    the ciphertexts and returns the decrypted plaintext which 
    has the highest English score.#>
    $candidates = @()
    foreach($string in $encrypted_strings) {
        $candidates += (Get-SingleCharXorBruteForce $string)
    }
    $high_score = $candidates.score | Sort-Object -Descending | Select-Object -First 1
...
$ciphertexts = Get-Content s1c04_input.txt 
$most_likely_plaintext = Detect-EncryptedText $ciphertexts
